...
In the following I show a case using the dNN algorithm. First some libraries and the Cars93 data are imported. The dNN function is created in "dnn.R". The Cars93 dataset is saved in "data".
```{r}
rm(list = ls())
suppressMessages(library(dplyr))
suppressMessages(library(data.table))
suppressMessages(require(MASS))
suppressMessages(source("dnn.R"))
data <- Cars93
```
...
## Run dNN with 2 to 4 predictor variables
...
This file is to demonstrate how to use dNN, but not to analyze the Cars93 data. Next, I will demonstrate using dNN on classification (coming soon).
